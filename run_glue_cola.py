...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="/home/data02/zj23/GaLore-master/datasets/GLUE/cola/train.csv", help="A csv or a json file containing the training data."
    )
    parser.add_argument(
        "--validation_file", type=str, default="/home/data02/zj23/GaLore-master/datasets/GLUE/sst2/dev.csv", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--max_length",
        type=int,
...
def main():
    ...
    def preprocess_function(examples):
        ...
        if "label" in examples:    #TODO
            if label_to_id is not None:
                # Map labels to IDs (not necessary for GLUE tasks)
                result["labels"] = [label_to_id[l] for l in examples["label"]]
            else:
                # In all cases, rename the column to labels because the model will expect that.
                result["labels"] = examples["label"]  #TODO
        return result
    ...
    if not args.enable_galore:
        ...
        for module_name, module in model.named_modules():
            ...
            galore_params_names.append(module_name)

        id_galore_params = [id(p) for p in galore_params]
        # galore_params_dic = defaultdict(dict)
        # for p in galore_params:
        #    for n, p2 in model.named_parameters():
        #        if id(p) == id(p2):
        #            galore_params_dic[p] = n
        #            break
        # make parameters without "rank" to another group
        regular_params = [p for n,p in model.named_parameters() if id(p) not in id_galore_params]
        # then call galore_adamw
        param_groups = [{'params': regular_params}, 
                        {'params': galore_params, 'param_names': galore_params_names, 'rank': args.lora_r, 'update_proj_gap': args.update_proj_gap, 'scale': args.galore_scale, 'proj_type': args.proj_type}]
    ...
    if args.task_name is not None:
        metric = evaluate.load("/home/data02/zj23/GaLore-master/evaluate-main/metrics/glue", "sst2")    #TODO
    else:
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        all_eval_metrics.append({'epoch': epoch, 'metric': eval_metric})
    ...
    if args.output_dir is not None:
        ...
        # 找出最高的accuracy和对应的epoch
        max_accuracy = max(result["metric"]["accuracy"] for result in all_eval_metrics)
        max_epoch = next(result["epoch"] for result in all_eval_metrics if result["metric"]["accuracy"] == max_accuracy)

        # 将最高accuracy和对应epoch添加到列表末尾
        all_eval_metrics.append({
            "epoch": max_epoch,
            "metric": {"best_accuracy": max_accuracy}
        })

        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        result_filename = f"all_results_{current_time}.json"
        with open(os.path.join(args.output_dir, result_filename), "w") as f:
            for item in all_eval_metrics:
