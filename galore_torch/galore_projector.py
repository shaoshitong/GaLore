...
class GaLoreProjector:
    def __init__(self, rank, verbose=False, update_proj_gap=200, scale=1.0, proj_type='std',layer_name=None):
        self.rank = rank
    ...
    def project(self, full_rank_grad, iter,c_grad=None):
        self.c_grad = c_grad
        if self.proj_type == 'std':
            if not hasattr(self, "c_grad_avg"):
                self.c_grad_avg = self.c_grad/240   #最后一个
            if full_rank_grad.shape[0] >= full_rank_grad.shape[1]:
                if self.ortho_matrix is None:
                    self.ortho_matrix = self.get_orthogonal_matrix(self.c_grad_avg, self.rank, type='right')
                low_rank_grad = torch.matmul(full_rank_grad, self.ortho_matrix.t().to(full_rank_grad.device.type))
            else:
                if self.ortho_matrix is None:
                    self.ortho_matrix = self.get_orthogonal_matrix(self.c_grad_avg, self.rank, type='left')
    ...
    def get_orthogonal_matrix(self, weights, rank, type):
        ...
        if min(matrix.shape[0], matrix.shape[1]) > 1650:
            U, s, Vh = rsvd(matrix, rank)
            print("Using rSVD")
        else:
            U, s, Vh = torch.linalg.svd(matrix, full_matrices = False)
            print("Using torch.linalg.svd")
