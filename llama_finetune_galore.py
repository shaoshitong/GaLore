...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="/home/zj23/data/GaLore-master/GaLore-master/ft-training_set/commonsense_15k.json",
    ...
    # galore_scale
    parser.add_argument("--galore_scale", type=float, default=4.0)
    # proj_type
...
def main():
    ...
    def tokenize(prompt, add_eos_token=True):
        ...
        if (
                ...
                and result["input_ids"].shape[1] < args.cutoff_len
                and add_eos_token
        ):
            result["attention_mask"] = torch.cat([result["attention_mask"], torch.ones((1, 1), dtype=torch.long)],
                                                 dim=1)
            result["input_ids"] = torch.cat([result["input_ids"], torch.tensor([[tokenizer.eos_token_id]])], dim=1)

        result["labels"] = result["input_ids"].clone()

        result["attention_mask"] = result["attention_mask"].squeeze()
        result["input_ids"] = result["input_ids"].squeeze()
        result["labels"] = result["labels"].squeeze()
    ...
    train_dataset = raw_datasets["train"].shuffle().map(generate_and_tokenize_prompt)
    ...
    # target_modules_list = ["q_proj", "v_proj"]
    galore_params = [b.weight for a, b in model.named_modules() if isinstance(b, torch.nn.Linear)][:-1]
    galore_params_names = [a for a, b in model.named_modules() if isinstance(b, torch.nn.Linear)][:-1]
